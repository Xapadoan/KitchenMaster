...
export class ShpLsPrsrMealMenuValidator
	implements IValidate<ShpLsPrsrMealMenu>
{
	validate(item: unknown): IResult<ShpLsPrsrMealMenu, ValidationError> {
		...
		const hasInvalidRecipeId = asMealMenu.RecipeIds.some(
			(id) => typeof id !== "string",
		);
...
export class ShpLsPrsrDayMenuValidator implements IValidate<ShpLsPrsrDayMenu> {
	...
	validate(item: unknown): IResult<ShpLsPrsrDayMenu, ValidationError> {
		...
		const breakfastResult = this.mealMenuValidator.validate(
			asDayMenu.Breakfast,
		);
		if (breakfastResult.isErr()) {
			return Result.err({
				...
				path: "Breakfast",
				error: breakfastResult.unwrapErr(),
			});
		}
...
export function ShpLsPrsrMenuCollectionAdapter(
	collection: ShpLsPrsrMenuCollection,
): WeekMenu {
	return {
...
export class ShpLsPrsrMenuCollectionValidator
	implements IValidate<ShpLsPrsrMenuCollection>
{
	constructor(private readonly validator: IValidate<ShpLsPrsrMealMenu>) {}
